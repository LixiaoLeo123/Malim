<script lang="ts">
  ...
  let layoutThresholds = { t_large: 1, t_wide: 1, t_tall: 1 };

  function calculateThresholds(arts: Article[]) {
    if (!arts || arts.length === 0) return { t_large: 1, t_wide: 1, t_tall: 1 };
    const scores = arts
  ...
  function getDifficultyRingClass(diff: number | null): string {
    ...
    if (diff < 0.3) return "ring-green-400/50 border-green-500/30 dark:ring-green-500/40 dark:border-green-400/20";
    if (diff < 0.6) return "ring-amber-400/50 border-amber-500/30 dark:ring-amber-500/40 dark:border-amber-400/20";
  ...
  async function loadSources() {
    try {
      sources = await invoke("get_sources_by_language", { lang: currentLang });
...
<div class="flex flex-col h-full bg-white relative dark:bg-zinc-950 overflow-hidden">
  <div class="flex flex-col border-b border-zinc-100 dark:border-zinc-800 z-40 bg-white/90 dark:bg-zinc-950/90 backdrop-blur">
    <div class="flex justify-between items-center p-4 pb-2">
      ...
      <div class="flex items-center space-x-2">
        <button
          ...
          title="Reset Feed"
        >
          <RotateCcw size={18} class="transition-transform duration-500 {isResetting ? '-rotate-180' : ''}" />
        </button>
        <button
        ...
        <div class="relative w-[110px]">
          ...
          >
            ...
            <ChevronDown size={14} class="transition-transform {showLangSelector ? 'rotate-180' : ''}" />
          </button>
          {#if showLangSelector}
            <div
    ...
    {#if sources.length > 0}
      <div class="flex overflow-x-auto whitespace-nowrap scrollbar-hide px-4 py-2 gap-2 pb-3">
        {#each sources as source}
          <button
            on:click={() => toggleSource(source.id)}
            class="px-3 py-1 text-xs rounded-full border transition-colors {selectedSourceIds.has(source.id)
  ...
  >
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 auto-rows-[140px] grid-flow-dense">
      {#each articles as article, i (article.url)}
        ...
        >
          <div class="absolute top-2 right-2 flex gap-1.5 z-10 opacity-90 group-hover:opacity-100 transition-opacity">
