<script lang="ts">
    ...
    let tagEditorArticleId: string | null = null;
    ...
    $: filterTagOptions = allTags
        .filter((t) => t.toLowerCase().startsWith(filterInput.toLowerCase()))
        .slice(0, 10);

    $: filteredArticles = activeFilterTag
        ? $articles.filter((a) => (a.tags ?? []).includes(activeFilterTag!))
        : $articles;
    ...
    function toggleStar(id: string) {
        articles.update((items) =>
            items.map((i) => (i.id === id ? { ...i, stared: !i.stared } : i)),
        );
    }
    ...
    let listLimit = 20;
    $: visibleArticles = filteredArticles.slice(0, listLimit);
    ...
    function handleListScroll(e: UIEvent) {
        ...
        if (
            ...
            if (listLimit < filteredArticles.length) {
                listLimit += 10;
            }
        }
    }
        ...
        <div class="flex items-center gap-1">
            {#if showSyncButton}
                <button
                    ...
                    class="p-2 rounded-xl transition-all duration-200 ease-out focus:outline-none {syncStatus ===
                    'syncing'
            ...
            <button
                ...
                class="p-2 ml-1 {isSelectMode
                    ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400'
                    : 'text-zinc-500 hover:bg-zinc-200/50 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-white'} active:scale-95 rounded-xl transition-all duration-200 shadow-sm"
                title="Select Mode"
            >
    ...
    {#if isSelectMode}
        ...
        >
            {#if showMultiDeleteConfirm}
                <span
                    class="text-xs font-bold text-red-500 tracking-wide uppercase shrink-0"
                >
            ...
            {:else}
                ...
                <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-1.5 {selectedArticleIds.size >
                    0
                        ? 'bg-red-500 hover:bg-red-600 text-white'
                        : 'bg-zinc-200 text-zinc-400 dark:bg-zinc-800 dark:text-zinc-600'} rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors disabled:cursor-not-allowed"
                >
                    <Trash2 size={14} />
                    Delete
                </button>
    ...
    >
        ...
        <div class="px-3 pt-3 mb-2 space-y-[2px]">
            ...
            >
                <div class="flex items-center gap-3">
                    <Globe
                        size={18}
                        class="text-blue-500/80 group-hover:text-blue-500 transition-colors"
                        opacity={0.9}
                    />
            ...
            >
                <div class="flex items-center gap-3">
                    <MessageCircle
                        size={18}
                        class="text-emerald-500/80 group-hover:text-emerald-500 transition-colors"
                        opacity={0.9}
                    />
            ...
            >
                <div class="flex items-center gap-3">
                    <Languages
                        size={18}
                        class="text-sky-500/80 group-hover:text-sky-500 transition-colors"
                        opacity={0.9}
                    />
            ...
            >
                <div class="flex items-center gap-3">
                    <BookOpenText
                        size={18}
                        class="text-amber-500/80 group-hover:text-amber-500 transition-colors"
                        opacity={0.9}
                    />
            ...
            >
                <div class="flex items-center gap-3">
                    <Sparkles
                        size={18}
                        class="text-indigo-500/80 group-hover:text-indigo-500 transition-colors"
                        opacity={0.9}
                    />
            ...
            >
                <div class="flex items-center gap-3">
                    <BarChart2
                        size={18}
                        class="text-orange-500/80 group-hover:text-orange-500 transition-colors"
                        opacity={0.9}
                    />
        ...
        <div class="px-3 mt-4">
            <div class="flex items-center justify-between px-2 pt-2 pb-2">
                ...
                <div
                    class="text-[10px] font-medium text-zinc-400/80 dark:text-zinc-600 bg-zinc-200/50 dark:bg-zinc-800/50 px-1.5 py-0.5 rounded-md"
                >
                    {activeFilterTag
                ...
                <div class="px-2 pb-2 space-y-2">
                    {#if activeFilterTag}
                        <div class="flex items-center gap-1.5 flex-wrap">
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400 shadow-sm"
                            >
                                <Hash size={10} />
                                <span>{activeFilterTag}</span>
                                <!-- <button
                                    on:click={() => setFilterTag(null)}
                            ...
                            <button
                                on:click={() => setFilterTag(null)}
                                class="text-[10px] font-semibold text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 uppercase tracking-wider transition-colors"
                            >
                                Clear
                            </button>
                        </div>
                    ...
                    <div class="relative">
                        <div
                            class="flex items-center gap-2 px-2.5 py-2 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200/60 dark:border-zinc-700/60 rounded-xl focus-within:border-indigo-300 dark:focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/10 transition-all"
                        >
                            <Tag size={13} class="text-zinc-400 shrink-0" />
                            <input
                                type="text"
                                bind:value={filterInput}
                                on:focus={() => (showFilterDropdown = true)}
                                on:blur={() =>
                                    setTimeout(
                                        () => (showFilterDropdown = false),
                                        180,
                                ...
                                class="flex-1 bg-transparent text-[12px] text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 outline-none min-w-0"
                            />
                            {#if filterInput}
                                <button
                        ...
                        {#if showFilterDropdown && filterTagOptions.length > 0}
                            <div
                                class="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-lg overflow-hidden z-30 max-h-40 overflow-y-auto no-scrollbar"
                            >
                                {#each filterTagOptions as tag}
                                    <button
                                    ...
                                    >
                                        <Hash size={12} class="text-zinc-400" />
                                        <span
                                            class="text-[12px] text-zinc-700 dark:text-zinc-300"
                                            >{tag}</span
                                        >
                                        <span
                                            class="ml-auto text-[10px] text-zinc-400 dark:text-zinc-500"
                                        >
                                            {$articles.filter((a) =>
                                                (a.tags ?? []).includes(tag),
                                            ).length}
            ...
            <div class="space-y-[3px]">
                {#each visibleArticles as article (article.id)}
                    <div
                        class="relative group rounded-xl select-none touch-manipulation overflow-hidden transition-all duration-200 ease-out active:scale-[0.99] active:bg-zinc-100 dark:active:bg-zinc-800 border {article.id ===
                        $activeArticleId
                            ? 'bg-purple-50/50 dark:bg-purple-500/10 border-purple-200/60 dark:border-purple-500/20 shadow-sm'
                            : 'bg-transparent border-transparent hover:bg-white dark:hover:bg-zinc-900 hover:border-zinc-200/50 dark:hover:border-zinc-800 hover:shadow-sm'}"
                    ...
                    >
                        {#if isSelectMode && article.status !== "parsing"}
                            <div class="absolute right-3 top-3 z-10">
                                {#if selectedArticleIds.has(article.id)}
                                    <div
                                        class="w-5 h-5 bg-indigo-500 rounded-[5px] flex items-center justify-center shadow-sm"
                                    >
                                        <CheckSquare
                                            size={14}
                                            class="text-white"
                                ...
                                {:else}
                                    <div
                                        class="w-5 h-5 bg-white border border-zinc-300 dark:border-zinc-700 dark:bg-zinc-800 rounded-[5px] shadow-sm"
                                    ></div>
                                {/if}
                            </div>
                        {/if}
                        ...
                        <div
                            class="absolute -left-10 -bottom-10 opacity-[0.08] dark:opacity-[0.12] pointer-events-none transition-all duration-300 group-hover:scale-110 group-hover:rotate-6 group-hover:opacity-[0.15] dark:group-hover:opacity-[0.2]"
                        >
                        ...
                        {#if article.stared}
                            <div
                                class="absolute left-0 top-0 bottom-0 w-[3px] bg-orange-400 rounded-l-xl z-10"
                            ></div>
                        {/if}

                        {#if article.status !== "parsing" && article.status !== "error" && article.readProgress > 0}
                            <div
                                class="absolute top-0 left-0 right-0 h-0.5 bg-zinc-100 dark:bg-zinc-800/80 overflow-hidden opacity-80 group-hover:opacity-100 transition-opacity z-10"
                            >
                                <div
                        ...
                        <div
                            class="px-3 py-2.5 flex flex-col gap-1.5 z-10 relative {article.stared
                                ? 'pl-4'
                                : ''}"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <h3
                                    class="font-semibold text-[13.5px] leading-snug line-clamp-2 {isSelectMode
                                        ? 'pr-8'
                                        : 'pr-1'} {article.id ===
                                    $activeArticleId
                                ...
                                {#if !isSelectMode}
                                    <div
                                        class="flex items-center gap-1.5 shrink-0 pt-0.5"
                                    >
                                        {#if article.stared}
                                            <button
                            ...
                            {#if (article.tags ?? []).length > 0}
                                <div class="flex flex-wrap gap-1 mt-0.5">
                                    {#each (article.tags ?? []).slice(0, 3) as tag}
                                        <span
                                            class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-semibold bg-indigo-50/80 text-indigo-500 dark:bg-indigo-500/10 dark:text-indigo-400 select-none"
                                        >
                                            <Hash size={8} />
                                            {tag}
                                        </span>
                                    ...
                                    {#if (article.tags ?? []).length > 3}
                                        <span
                                            class="text-[9px] text-zinc-400 dark:text-zinc-500"
                                            >+{(article.tags ?? []).length -
                                                3}</span
                                        >
                                    {/if}
                            ...
                            >
                                ...
                                {:else}
                                    ...
                                    >
                                        ...
                                        <button
                                            on:click|stopPropagation={() =>
                                                openTagEditor(article.id)}
                                            class="flex-1 flex flex-col items-center justify-center gap-1 rounded-lg text-indigo-600 bg-indigo-50/80 hover:bg-indigo-500 hover:text-white dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:bg-indigo-500 dark:hover:text-white transition-all shadow-sm"
                                            title="Tag"
                                        >
                                            <Tag size={18} />
                                            <span
                ...
                {#if visibleArticles.length === 0}
                    <div class="py-8 text-center px-4">
                        <div
                            class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-zinc-100 dark:bg-zinc-800 mb-3 text-zinc-400"
                        >
                            <Plus size={24} />
                        </div>
                        <h4
                            class="text-[13px] font-semibold text-zinc-700 dark:text-zinc-300 mb-1"
                        >
                            {activeFilterTag
                        ...
                        {#if activeFilterTag}
                            <button
                                on:click={() => setFilterTag(null)}
                                class="mt-3 px-3 py-1.5 text-[11px] font-semibold rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:bg-indigo-500/20 transition-colors"
                            >
                                Clear Filter
                            </button>
                        {/if}
...
<TagEditorModal
    bind:opened={showTagEditor}
    articleId={tagEditorArticleId}
    {allTags}
/>
