...
pub async fn send_message(
    ...
    let handler = &state.memory_handler;
    let main_api = (main_api_key.as_str(), main_api_url.as_str(), main_model_name.as_str());
    let shadow_api = (shadow_api_key.as_str(), shadow_api_url.as_str(), shadow_model_name.as_str());
    let embed_api = (embed_api_key.as_str(), embed_api_url.as_str(), embed_model_name.as_str());

    let result = handler.handle_message(user_input, main_api, shadow_api, embed_api).await;
    dbg!(&result);
    result
}
...
pub async fn trigger_proactive(
    ...
    let handler = &state.memory_handler;
    handler.trigger_proactive_message(message, scheduled_time).await
}
